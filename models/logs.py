...
def create_log(db: Session, asset_id: int=0, control_box_id: int=0, battery_id: int=0, inverter_id: int=0, sensor_id: int=0, port_id: int=0, ip_address: str=None, data: str=None, status: int=0):
    log = Log(asset_id=asset_id, control_box_id=control_box_id, battery_id=battery_id, inverter_id=inverter_id, sensor_id=sensor_id, port_id=port_id, ip_address=ip_address, data=data, status=status, created_at=get_laravel_datetime(), updated_at=get_laravel_datetime())
    db.add(log)
    db.commit()
    db.refresh(log)
    return log
